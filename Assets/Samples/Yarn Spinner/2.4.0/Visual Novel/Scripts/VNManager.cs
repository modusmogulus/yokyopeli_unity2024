...
namespace Yarn.Unity.Example {
    ...
    public class VNManager : DialogueViewBase
    {
		...
		public List<Texture2D> loadCharacters = new List<Texture2D>(); // t. aino
		...
		List<Texture2D> charTextures = new List<Texture2D>(); // t. aino
		...
		{
			/*
			MainGameObject.Instance.displayedCharacterTexture = FetchAsset<Texture2D>(character);
			MainGameObject.Instance.setCharacterTexture(FetchAsset<Texture2D>(character));
			*/
			//TIP: Right click on texture and click copy path then insert that to yarn!
			MainGameObject.Instance.setCharacterTexture(character);
		}
		public void RawImgShow()
		{
			MainGameObject.Instance.setShowCharacter(true);
		}

		public void RawImgHide()
		{
		...
		T FetchAsset<T>( string assetName ) where T : UnityEngine.Object {
			...
			if ( typeof(T) == typeof(Sprite) ) {
				...
				{
					foreach (var spr in loadCharacters)
					{
						if (spr.name == assetName)
						{
							return spr as T;
